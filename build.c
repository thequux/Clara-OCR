        ...
        */
        if (m->tr == NULL) {
            db("oops.. untransliterated symbol classified as accent");
                 ...
                 (box_dist(ps[i],b,NULL) < ((mc[b].b-mc[b].t)/2))) {
        ...
        */
        if (m->tr == NULL) {
		 ...
		 && (mc[b].l < mc[i].r) && (mc[i].l < mc[b].r) 
            ...
            else if ((box_dist(ps[i],b,NULL) < ((mc[b].b-mc[b].t)/2))) {
...
*/
int bl_geo(int a,float *sl,int *at,int *ab,int *mh,float *mx,float *my)
{
    /* alignment */
    int as,xh,bl,ds;
...
int build(int reset)
{
    ...
    */
    if (st == 0) {
        ...
        */
        for (m=0; m<=tops; ++m) {
            mc[m].E = mc[m].W = mc[m].sw = mc[m].sl = mc[m].bs = -1;
        }
