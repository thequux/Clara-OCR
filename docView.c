...
static void copy_bm_to_buffer(guchar* src, guchar* dst, int w, int h, int stride);
...
static void clara_doc_view_size_request(GtkWidget* widget, GtkRequisition *req);
static void clara_doc_view_size_allocate(GtkWidget* widget, GtkAllocation *alloc);
static void clara_doc_view_set_property(GObject* object, guint property_id,
                                        const GValue* value, GParamSpec *pspec);
static void clara_doc_view_get_property(GObject* object, guint property_id,
...
static void clara_doc_view_set_scroll_adjustments(ClaraDocView* self, GtkAdjustment* hpos, GtkAdjustment *vpos);
static void clara_doc_view_set_adjustment(ClaraDocView* self, GtkAdjustment* adj, GtkOrientation orientation, gboolean set_values);
...
static void clara_doc_view_class_init(ClaraDocViewClass *klass) {
	...
	widget_class->button_press_event = clara_doc_view_button_press_cb;
        ...
        klass->set_scroll_adjustments = clara_doc_view_set_scroll_adjustments;
        
        widget_class->set_scroll_adjustments_signal = 
                g_signal_new(g_intern_static_string("set-scroll-adjustments"),
                             G_OBJECT_CLASS_TYPE(gobject_class),
                             G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION,
                             G_STRUCT_OFFSET(ClaraDocViewClass, set_scroll_adjustments),
                             NULL, NULL,
...
static void clara_doc_view_set_adjustment(ClaraDocView* self, GtkAdjustment* adj, GtkOrientation orientation, gboolean set_values) {
        ...
        if (*aptr != NULL) {
                g_signal_handlers_disconnect_by_func(*aptr, clara_doc_view_viewport_changed, self);
                g_object_unref(*aptr);
                *aptr = NULL;
        }
        if (adj == NULL)
                adj = GTK_ADJUSTMENT(gtk_adjustment_new(0,0,0,
                                                        0,0,0));
        *aptr = adj;
...
static void clara_doc_view_set_property(GObject* object, guint property_id,
        ...
        case PROP_HADJUSTMENT:
                clara_doc_view_set_adjustment(self, g_value_get_object(value), 
...
static void clara_doc_view_get_property(GObject* object, guint property_id,
        ...
        case PROP_ZOOM_ADJUSTMENT:
                ...
                break;
        case PROP_ZOOM:
                g_value_set_float(value,gtk_adjustment_get_value(priv->zoom_adj));
                break;
        default:
                G_OBJECT_WARN_INVALID_PROPERTY_ID (object, property_id, pspec);
                break;
...
static void clara_doc_view_adjust_position(ClaraDocView* self, GtkAnchorType anchor) {
        ...
        switch(anchor) {
        case GTK_ANCHOR_NORTH_WEST: xanc = 0; yanc = 0; break;
        case GTK_ANCHOR_NORTH:      xanc = 1; yanc = 0; break;
        case GTK_ANCHOR_NORTH_EAST: xanc = 2; yanc = 0; break;
        case GTK_ANCHOR_WEST:       xanc = 0; yanc = 1; break;
        case GTK_ANCHOR_CENTER:     xanc = 1; yanc = 1; break;
        case GTK_ANCHOR_EAST:       xanc = 2; yanc = 1; break;
        case GTK_ANCHOR_SOUTH_WEST: xanc = 0; yanc = 2; break;
        case GTK_ANCHOR_SOUTH:      xanc = 1; yanc = 2; break;
        case GTK_ANCHOR_SOUTH_EAST: xanc = 2; yanc = 2; break;
        default:
        ...
        if (priv->hadjustment != NULL) {
                ...
                gtk_adjustment_configure(priv->hadjustment, 
                                         (priv->keep_position 
                                          ? CLAMP(xpos + xanc * (page_width - hsize) / 2.0, 0, XRES - page_width) 
                                          : 0),
                                         0, XRES,
        ...
        if (priv->vadjustment != NULL) {
                ...
                gdouble vsize = gtk_adjustment_get_page_size(priv->vadjustment);
                
                gtk_adjustment_configure(priv->vadjustment,
                                         (priv->keep_position 
                                          ? CLAMP(ypos + yanc * (page_height - vsize) / 2.0, 0, YRES - page_height) 
                                          : 0),
                                         0, YRES,
...
static void copy_bm_to_buffer(guchar* src, guchar* dst,
        ...
        for (j = 0; j<h; j++) {
                for (i = 0; i < w; i++) {
                        ...
                        dst[j*stride + i] = (src[j*src_stride + (i >> 3)] & (128 >> (i % 8))) ? 0xff : 0;
                }
        }
}

static void cairo_ellipse(cairo_t* cr, gdouble x, gdouble y, gdouble width, gdouble height) {
	cairo_save (cr);
...
static void draw_symbol(cairo_t* cr, drawmode_t mode, sdesc* sym) {
	...
	case DV_CLOSURE_UNCERTAIN:
                ...
                for (cn = 0; cn < sym->ncl; cn++) {
                        ...
                        copy_bm_to_buffer(closure->bm, buffer, w, h, stride);
                        cairo_surface_t *surface = cairo_image_surface_create_for_data(buffer, CAIRO_FORMAT_A8, w, h, stride);
