...
void html2ge(char *ht, int lm) {
        ...
        for (t = ht; (el = get_tag(&t)) != CML_END;) {
                ...
                /* line break */
                else if ((el == CML_TAG) && (strcmp(tag, "BR") == 0)) {
                ...
                /* select begin */
                else if ((el == CML_TAG) && (strcmp(tag, "SELECT") == 0)) {
                ...
                /* select option */
                else if ((el == CML_TAG) && (strcmp(tag, "OPTION") == 0)) {
                ...
                /* IMG SRC */
                else if ((el == CML_TAG) && (strcmp(tag, "IMG") == 0)) {
                        int n, w, h;
                ...
                if (href != NULL) {
                        ...
                        if (lhs < (a = strlen(href) + 1))
                                last_href = c_realloc(last_href, lhs =
                                                      a + 32, NULL);
...
void totext(const char *fmt, ...) {
        ...
        for (f = 0; f == 0;) {
                ...
                /* try to write */
                va_start(args, fmt);
                n = vsnprintf(text + topt + 1, textsz - topt - 1, fmt,
                              args);
                va_end(args);
...
void pm_tt(char *n, char *t, char *v, int e) {
        totext("<TR><TD BGCOLOR=DARKGRAY>%s</TD><TD>", t);
        if (e)
                totext("<INPUT TYPE=TEXT SIZE=20 VALUE=\"%s\" NAME=%s>", v,
                       n);
        else
                totext("%s", v);
        totext("</TD></TR>\n");
...
void pm_tf(char *n, char *t, float v, int e) {
        totext("<TR><TD BGCOLOR=DARKGRAY>%s</TD><TD>", t);
        if (e)
                totext("<INPUT TYPE=TEXT SIZE=8 VALUE=%3.2f NAME=%s>", v,
                       n);
        else
                totext("%3.2f", v, n);
        totext("</TD></TR>\n");
...
void pm_tli(char *n, char *t, int v, int e) {
        totext("<TR><TD BGCOLOR=DARKGRAY>%s</TD><TD>", t);
        if (e)
                totext("<INPUT TYPE=NUMBER SIZE=8 NAME=%s VALUE=%d>", n,
                       v);
        else
                totext("%d", v);
        totext("</TD></TR>\n");
...
void pm_tlf(char *n, char *t, float v, int e) {
        totext("<TR><TD BGCOLOR=DARKGRAY>%s</TD><TD>", t);
        if (e)
                totext("<INPUT TYPE=NUMBER SIZE=8 NAME=%s VALUE=%3.2f>", n,
                       v);
        else
                totext("%3.2f", v);
        totext("</TD></TR>\n");
        ...
        for (t = m->tr, n = 0; t != NULL; t = t->nt, ++n) {
                ...
                for (v = t->v; v != NULL; v = (vdesc *) (v->nv)) {
                        if ((0 <= v->act) && (v->act <= topa)) {
                                ...
                                totext("<LI>by %s (%s) ", a->r,
                                       typename(a->rt));
                                /*
                                   if (v->orig == REVISION)
                                   totext("[<A HREF=nullify/%d>nullify</A>]\n",v->act);
                                 */
                                totext("<LI>from %s at %s", a->sa,
                                       ctime(&(a->dt)));
                                totext("</UL>\n");
                        }
                }
...
void mk_page_doubts(void) {
        ...
        for (i = 0; i <= topw; ++i) {
                if ((w[i] == 10) || (w[i] == 11)) {
                        totext
                            ("<BR><A HREF=wfocus/%d><IMG SRC=word/%d></A>\n",
                             i, i);
...
void mk_page_output(output_encap encap) {
        ...
        if (encap == OE_FULL_HTML) {
                totext
                    ("<HTML><HEAD><TITLE>OCR Output</TITLE></HEAD><BODY>\n");
        }
...
void mk_pattern_list(void) {
        ...
        /* HTML headings */
        totext
            ("<HTML><HEAD><TITLE>List of Patterns</TITLE></HEAD><BODY>\n");
...
void mk_pattern_types(void) {
        ...
        if ((1 <= cpt) && (cpt <= toppt)) {
                ...
                /* accounting */
                {
                        ...
                        a = pt[cpt].acc;
                        pm_cbox("AA", a & 1,
                                "Account accented letters<BR>");
                        pm_cbox("AD", a & (1 << NUMBER),
                                "Account Decimal digits<BR>");
                        pm_cbox("AL", a & (1 << LATIN),
                                "Account Latin letters<BR>");
                        pm_cbox("AG", a & (1 << GREEK),
                                "Account Greek letters<BR>");
...
void mk_pattern_props(void) {
        ...
        {
                if (s->act >= 0) {
                        totext("From <A HREF=act/%d>act %d</A><BR>",
                               s->act, s->act);
                }
...
void mk_pattern_action(void) {
        ...
        totext
            ("<TR><TD></TD><TD>previously found similar to one pattern</TD>\n");
        totext("</TABLE>");
        ...
        totext("<BR>Select the best alternative:<BR>\n");
        {
                ...
                snprintf(s, 80, "Both pattern and symbol are '%s'<BR>",
                         r_tr_old);
                s[80] = 0;
                ...
                snprintf(s, 80,
                         "The symbol is '%s', the pattern is '%s'<BR>",
                         r_tr_new, r_tr_old);
...
void mk_tune(void) {
        ...
        totext("<BR>");
        pm_cbox("PPD", pp_deskew,
                "Use deskewer (set number of columns, see n. 6)<BR>");
        pm_cbox("PPDA", pp_deskew_accurate, "Accurate deskew<BR>");
        pm_cbox("PPB", pp_bal,
                "Apply balance (check to change thres. factor)<BR>");

        /* segmentation */
        pm_hl("2. Binarization/segmentation");
        totext("<BR>");
        pm_cbox("DR", pp_double, "Double resolution<BR>");
        pm_radio("TT", bin_method == 1, 1,
                 "Use manual global thresholder<BR>");
        pm_radio("TT", bin_method == 2, 2,
                 "Use histogram-based global thresholder<BR>");
        pm_radio("TT", bin_method == 3, 3, "Use local binarizer<BR>");
        pm_radio("TT", bin_method == 4, 4,
                 "Strong mode (unfinished - don't try)<BR>");
        totext("<TABLE>\n");
        ...
        totext
            ("<BR><INPUT TYPE=RADIO NAME=C %s VALUE=1>Skeleton-based classifier\n",
             C);
        C = (classifier == CL_BM) ? "CHECKED" : "";
        totext
            ("<BR><INPUT TYPE=RADIO NAME=C %s VALUE=2>Border mapping classifier (experimental)\n",
             C);
        C = (classifier == CL_PD) ? "CHECKED" : "";
        totext
            ("<BR><INPUT TYPE=RADIO NAME=C %s VALUE=3>Pixel distance classifier\n",
             C);
        totext("<BR>\n");
        ...
        totext("<BR><TABLE>\n");
        pm_tl("MWD", "max word distance as percentage of x_height", m_mwd,
              1);
        pm_tl("MSD", "max symbol distance as percentage of x_height",
              m_msd, 1);
        pm_tf("DD", "dot diameter (mm)", DD, 1);
        pm_tl("MAE", "max alignment error as percentage of DD", m_mae, 1);
        pm_tl("DS", "descent (relative to baseline) as percentage of DD",
              m_ds, 1);
        pm_tl("AS", "ascent (relative to baseline) as percentage of DD",
              m_as, 1);
        pm_tl("XH", "x_height (relative to baseline) as percentage of DD",
              m_xh, 1);
        ...
        pm_tlc("S", "columns separated by vertical line", (SL != 0),
               !PG_AUTO);
...
void mk_tune_skel(void) {
        ...
        /* HTML headings */
        totext
            ("<HTML><HEAD><TITLE>Skeleton Tuning</TITLE></HEAD><BODY>\n");
        ...
        if ((0 <= cdfc) && (cdfc <= topp)) {
                ...
                totext("%d (%dx%d pixels)", q, pattern[cdfc].bp,
                       pattern[cdfc].sp);
                totext("</TD></TR>\n");
        }
                        ...
                        for (i = 0; (i <= topge); ++i) {
                                ...
                                if (ge[i].type == GE_INPUT) {
                                        if (strcmp(ge[i].arg, "SA") == 0)
                                                SA = atoi(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "RR") ==
                                                 0)
                                                RR = atof(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "MA") ==
                                                 0)
                                                MA = atof(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "MP") ==
                                                 0)
                                                MP = atoi(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "ML") ==
                                                 0)
                                                ML = atof(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "MB") ==
                                                 0)
                                                MB = atoi(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "RX") ==
                                                 0)
                                                RX = atoi(ge[i].txt);
                                        else if (strcmp(ge[i].arg, "BT") ==
                                                 0)
                                                BT = atoi(ge[i].txt);
                                }
                ...
                for (i = 0; (i <= topge); ++i) {
                        ...
                        /* the checkboxes... */
                                ...
                                /* per-symbol per-method flag */
                                        ...
                                        if (j >= l) {
                                                fatal(IE,
                                                      "unexpected PATTERN TYPES field name");
                                        }
                                        m = atoi(g);
                                        c = atoi(g + j + 1);
                                        if (ge[i].iarg == 2) {
...
void form_auto_submit(void) {
         ...
         */
        if ((dw[PAGE].v) && ((curr_mc < 0) || (tops < curr_mc))) {
        ...
        if (form_changed &&
            ((CDW == PAGE_SYMBOL) ||
             ...
             (CDW == PATTERN_TYPES) ||
             (CDW == PATTERN_ACTION) ||
             (CDW == TUNE) || (CDW == TUNE_SKEL) || (CDW == TUNE_ACTS)))
...
void html_action(int x, int y) {
        ...
        if (ge[curr_hti].type == GE_STEPPER) {
                ...
                if (f) {
                        snprintf(c->txt, c->tsz, "%3.2f",
                                 atof(c->txt) + ((float) d) / m_fs);
                } else {
