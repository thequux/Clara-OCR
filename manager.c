...
char *LOGP = "/var/manager/log",
    ...
    *DU = "/usr/bin/du", *GZIP = "/usr/bin/gzip", *MNTCD = "/mnt/cdrom";
...
int toburn_full = 0,
    burn_st = 0,
    burn_rc, burn_speed, burn_fail, fatal_failure = 0, large_disk = 1;
...
*/
int testing = 0,
    have_kbd = 0, have_oper = 1, have_scanner = 1, silently = 0;
...
#define FAIL_SND 3
#define WARN_SND 4
int sound = 0, sound_st, sound_rep, sound_all, term6 = -1;
...
void warn(int f, char *m, ...) {
        ...
        /* rotate logfile */
        if ((LOG != NULL) && (stat(LOGP, &fb) == 0) &&
            (fb.st_size > 1024 * 1024)) {
                fclose(LOG);
                rename(LOGP1, LOGP2);
                rename(LOGP, LOGP1);
                LOG = fopen(LOGP, "w");
...
void hd(int op) {
        ...
        if (op == 1) {
                ...
                /* move */
                if (e1) {
                        snprintf(path, MAXPATH, "%s %s/* %s", MV, TOBURN1,
                                 TOBURN);
                        path[MAXPATH] = 0;
                        system(path);
                }
                if (e2) {
                        snprintf(path, MAXPATH, "%s %s/* %s", MV, TOBURN2,
                                 TOBURN);
                        path[MAXPATH] = 0;
                        system(path);
                }
...
int make_beep(void) {
        ...
        /* sound "EMERGENCY" (bad results) */
                ...
                if (sound_st < 80) {
                        nt = mybeep((2 - 1 / (((float) sound_st)) / 2 +
                                     0.5) * 400, 15);
                        nt -= 5000;
                        ++sound_st;
                }
...
*/
int consist_sp(int x0, int y0, int xsz, int ysz, int res, int fid,
               int mode) {
        if ((x0 < 0) || (230 < x0) ||
            (y0 < 0) || (350 < y0) ||
            (xsz < 0) || (230 < (x0 + xsz)) ||
            (ysz < 0) || (350 < (y0 + ysz)) ||
            (res < 0) || (600 < res) ||
            (fid < 0) || (1000000 < res) || (mode < 0) || (2 < mode)) {
        ...
        /* yes, someone exited */
                ...
                if (p == scan_pid) {
                        ...
                        if (stat(NEWPAGE, &fb) < 0) {
                                warn(0,
                                     "scanimage failed (cannot stat file)");
                        }
                        ...
                        if ((WIFEXITED(s) == 0) || (fb.st_size < size)) {
                                ...
                                if (fb.st_size < size)
                                        warn(0,
                                             "scanimage failed (file too small)");
                                else
                                        warn(0,
                                             "scanimage failed (as reported)");

                                /* unlink */
                                if (unlink(NEWPAGE) < 0)
                                        warn(1, "could not unlink %s",
                                             NEWPAGE);
                        }
...
void burn_cd(void) {
                            ...
                            ((strcmp(a + t - 7, ".pbm.gz") == 0) ||
                             ...
                             (strcmp(a + t - 7, ".pnm.gz") == 0))) {
                                ...
                                *sz += fb.st_size;
                                ...
                                if (rename(f, h) < 0) {
                                        warn(1,
                                             "could not move file while dividing");
                                }
                        }
                }
                closedir(D);
                ...
                if ((sz1 > 0) &&
                    (((ie = stat(IMAGE1, &fb)) < 0) ||
                     (fb.st_size < sz1))) {
                        ...
                        if (ie == 0)
                                warn(0,
                                     "image 1.raw not ok, waiting >%d found %d",
                                     sz1, fb.st_size);
                        else
         ...
         */
                ...
                if ((!burn_fail) && (sz2 > 0)) {
                         ...
                         */
                        for (retries = 3, r[0] = 0;
                             (r[0] == 0) && (retries > 0); --retries) {

                                warn(0, "will run cdrecord -msinfo");

                                /* run cdrecord -msinfo */
                                snprintf(path, MAXPATH, "%s -msinfo %s",
                                         CDRECORD, cdrecord_dev);
                                path[MAXPATH] = 0;
                                F = popen(path, "r");
                                fgets(r, 255, F);
                                pclose(F);
                        ...
                        if ((!burn_fail) && (sz2 > 0)) {
                                ...
                                if (*cddev == '=')
                                        ...
                                        warn(1,
                                             "invalid cdrecord device %s",
                                             cdrecord_dev);

                                warn(0,
                                     "ready to create the second image");
                ...
                if ((sz2 > 0) &&
                    (((ie = stat(IMAGE2, &fb)) < 0) ||
                     (fb.st_size < sz2))) {
                        ...
                        if (ie == 0)
                                warn(0,
                                     "image 2.raw not ok, waiting >%d found %d",
                                     sz2, fb.st_size);
                        else
         ...
         */
                ...
                if ((!burn_fail) && ((sz1 > 0) || (sz2 > 0))) {
                        ...
                        /* mount CD */
                        system
                            ("/bin/mount -t iso9660 /dev/scd0 /mnt/cdrom");
                        mnt = 1;
         ...
         */
                ...
                if ((sz1 > 0) || (sz2 > 0)) {
                        ...
                        if ((burn_fail) || (burn_rc != 0)) {
                                if (burn_fail) {
                                        warn(0,
                                             "CD is not OK (early failure)");
                                } else {
                                        warn(0, "CD is not OK (code %d)",
                                             burn_rc);
                                        burn_fail = 1;
                                }
                        }
                ...
                if (burn_fail == 0) {
                        ...
                        warn(0,
                             "finished burning procedure (failure, status is %d)",
                             burn_fail);
                        make_sound(FAIL_SND);
                }
        }
...
int main(int argc, char *argv[]) {
        ...
        for (f = ni = op = l = 0; f == 0;) {
                ...
                if (r > 0) {
                        ...
                        warn(0, "read %c from keyboard",
                             map[*((unsigned char *) &kbuf)]);
