...
struct barcode_symbology {
        ...
        int (*verify_checksum) (struct barcode * bp,
...
static struct barcode_symbology c39 =
    { 2, c39_codes, 0, c39_verify_checksum };
...
static int read_ocr(FILE *fp, struct barcode *bp) {
        char *sp;
        int c;
        long offset;
        size_t length;
...
static int read_scanline(FILE *fp, struct barcode *bp) {
        char *sp;
        int c;
        long offset;
        size_t length;
...
convert_scanline(struct barcode *bp, struct barcode_symbology *bsp) {
        ...
        while ((c = *sp) != '\0') {
                ...
                if (!
                    (wp =
                     (struct width *) calloc(1, sizeof(struct width)))) {
                        for (idx = 0; idx < 2; idx++)
                                for (np = NULL, cp = head[idx]; cp;
                                     cp = np) {
                                        np = cp->next;
                                        free(cp);
                                }
...
decode_symbology(struct barcode *bp, struct barcode_symbology *bsp,
        ...
        while (symbol[symbol_length]) {
                ...
                while (bcp->encoding && symbol_length >= bcp->nbits) {
                        if (strncmp(bcp->encoding, symbol, bcp->nbits) ==
                            0)
                                break;
                        bcp++;
                }
                ...
                if ((bcp->flags.start && symbol != bp->encoded)
                    || (bcp->flags.stop &&
                        symbol !=
                        &bp->encoded[bp->encoded_length -
                                     symbol_length])) {
                        for (tbcp = bcp + 1; tbcp->encoding; tbcp++)
                                if (strncmp
                                    (tbcp->encoding, symbol,
                                     symbol_length) == 0)
                                        break;
                ...
                case bc_numeric_type:
                        ...
                        if ((unsigned int) bcp->sets[charset].c > 10)
                                bp->decoded[bp->decoded_length++]
                                    = '0' +
                                    ((unsigned int) bcp->sets[charset].c /
                                     10);
                        bp->decoded[bp->decoded_length++]
                            = '0' +
                            ((unsigned int) bcp->sets[charset].c % 10);
                        break;
                ...
                if (!start)
                        ...
                        if (with_checksum || bsp->checksum_required) {
                                bp->decoded_length =
                                    previous_decoded_length;

                                if ((*bsp->verify_checksum) (bp,
                                                             codes) < 0) {
                                        free(codes);
       ...
       int with_checksum) {
        ...
        if (bsp) {
                ...
                for (idx = 0; idx < bp->encoded_length / 2; idx++) {
                        c = bp->encoded[idx];
                        bp->encoded[idx] =
                            bp->encoded[bp->encoded_length - 1 - idx];
        ...
        do {
                ...
                for (;;) {
                        ...
                        if (result == 0)
                                ...
                                fprintf(stderr,
                                        "%s: <%s> contains an invalid barcode.\n",
                                        MyName, filename);
                                if (fp != stdin)
                                        fclose(fp);
                                exit(1);
                        ...
                        fwrite(barcode.decoded, barcode.decoded_length, 1,
                               stdout);
