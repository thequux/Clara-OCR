...
void extend(bdesc *b,int x,int y,int w,int h)
{
    ...
    if ((dx < 0) || (dy < 0) || (w < b->w) || (h < b->h)) {
        fatal(IE,"bad extend call dx=%d dy=%d w=%d h=%d ow=%d oh=%d",
                  dx,dy,w,h,b->w,b->h);
    }
                ...
                for (k=0; k<FS; ++k) {
                    for (l=0; l<FS; ++l) {
                        if (pix(cmp_bmp,BLS,k,l)) {
                            pixmap[(l+cmp_bmp_y0)*XRES+k+cmp_bmp_x0] = 255;
                        }
                    }
                }
...
int pbm2bm(char *f,int reset)
{
    ...
    */
    ...
    if (reset) {
        ...
        */
        for (n=rc=0, w=h=0; (n<4) && ((c=zfgetc(F)) != EOF); ) {
    ...
    */
    {
        ...
        if ((batch_mode == 0) && ((n % DPROG) == 0)) {
            snprintf(mba,MMB,"now reading line %d/%d of file %s",n,h,pagebase);
            show_hint(0,mba);
        }
        ...
        for (i=1; i<=w; ++i) {
            if (((j=ll[i]) >= 0) && (ab[j].x>=0) && (ab[j].y<0)) {
                ...
                if (localbin &&
                    ...
                    */
                    t = byteat(ab[j].w);
                    for (v=0; (f==0) && (v<ab[j].h); ++v) {
                        m = 128;
                        p = ((unsigned char *)ab[j].b) + v*t;
                        for (u=0; (f==0) && (u<ab[j].w); ++u) {
                            if (*p & m) {
                                int x,y;
        ...
        */
        ...
        for (i=1; i<=w; ++i) {
            if ((k=kl[i]) >= 0) {
                ...
                */
                if (i-1 < ab[k].x)
                    if ((j-1)-ab[k].x+1 > ab[k].w)
                        extend(ab+k,i-1,ab[k].y,(j-1)-(i-1)+1,ab[k].h+1);
                    else
                        extend(ab+k,i-1,ab[k].y,ab[k].w+ab[k].x-(i-1),ab[k].h+1);
                else if ((j-1)-ab[k].x+1 > ab[k].w)
                    extend(ab+k,ab[k].x,ab[k].y,(j-1)-ab[k].x+1,ab[k].h+1);
                else
                ...
                */
                p = ab[k].b + (n-ab[k].y)*byteat(ab[k].w) + (i-1-ab[k].x)/8;
...
*/
void clip_cl(unsigned char *rw,int bpl,cldesc *y,int l,int r,int t,int b,int RWX,int RWY)
{
    int ia,ib,ja,jb;
    int i,j;
    unsigned char *p;
...
int wrzone(char *s1,int fz)
{
    ...
    if (s1 != NULL) {
        ...
        if (stat(s1,&buf) == 0) {
            show_hint(2,"write zone failure: a file %s already exists",s1);
            return(0);
        }
    }
    ...
    */
        ...
        for (k=0; k<list_s_sz; ++k) {
                ...
                */
                ...
                for (i=ia; i<=ib; ++i) {
                    for (j=ja; j<=jb; ++j) {
                        ...
                        li = l+i-mc[k].l;
                        lj = t+j-mc[k].t;
                        if ((0<li) && (li<FS) && (0<lj) && (lj<FS)) {
                            if (pix(pattern[mc[k].bm].b,FS/8,li,lj))
                                *p |= n;
                        }
                    }
                }
            ...
            /* dump the symbol on a per-closure basis */
                ...
                for (n=0; n<y->ncl; ++n) {
                    clip_cl(rw,bpl,cl+(y->cl)[n],l,r,t,b,RWX,RWY);
                }
            }
        }
    }
