...
void h_print(float *h, int width) {
        ...
        for (l = 1, M = h[0]; l <= MAXVAL; l++)
                if (h[l] > M)
                        M = h[l];
        for (l = 0; l <= MAXVAL; l++)
                if (h0[(int) l] != 0)
                        printf("%4d %6.0f %s\n", (int) l, h0[(int) l],
                               graf(h0[(int) l], 0, M, 70));
}
...
#define NP 10
        ...
        N = n = s = q = delta = 0;
        for (dy = YRES / 20; (delta >= n * Delta) && dy >= 1; dy *= 0.618) {
                l = (N > 0 ? s / N : -1);
                for (x = 3 * w / 2; x + 3 * w / 2 < XRES; x += 3 * w)
                        for (y = h + dy / 2; y + h < YRES; y += dy) {

                                /* m = min */
                                for (m = MAXVAL, i = c = 0, t = 0; c < w;
                                     c += dx, i++)
                                        if (m >
                                            (t =
                                             *PIX((int) (x + c),
...
#define R 0.618034
        ...
        D = 0;
        x1 = MaxSkew, x0 = -x1;
        for (x1 = (x1 - x0) * R + x0, y1 = skew_score(x1, D),
             x0 = (x1 - x0) * R + x0, y0 = skew_score(x0, D);
             fabs(x1 - x0) > Eps;
             D = (pp_deskew_accurate ? 0 : fabs(y1 - y0) / 30)
            ) {
                if (y0 < y1)
                        x0 = x1 / R - x0 * R, y0 = skew_score(x0, D);
                else if (y0 > y1)
                        x1 = x0 / R - x1 * R, y1 = skew_score(x1, D);
                else
                        break;
        }
        return (x0 + x1) / 2;
...
void pp_shear_y(float f) {
        ...
        for (y = 0; y + s < YRES; y++)
                for (x = x0; x < x1; x++) {
                        D = y + d[x];
                        *PIX(x, y) =
                            (float) 0.5 + ITP(*PIX(x, D), *PIX(x, (D + 1)),
                                              m1[x]);
                }
...
void pre_balance() {
        ...
        /* init icon */
        /* steps for icon */
        Ry = (LS + LH) * DENSITY / 25.4;
        Rx = (LW * 2) * DENSITY / 25.4;
        ...
        for (y = 0; y < Ih; y++) {
                for (x = 0; x < Iw; x++) {
                        ...
                        x1 = (Dx + x * Rx + Rx >=
                              XRES ? XRES : Dx + x * Rx + Rx);
                        y1 = (Dy + y * Ry + Ry >=
                              YRES ? YRES : Dy + y * Ry + Ry);
                        for (i = 0; i <= MAXVAL; i++)
                                h0[i] = 0;
                        for (j = y0; j <= y1; j += dy)
                                for (i = x0, p = PIX(x0, j); i <= x1;
                                     i += dy, p += 2)
                                        h0[*p]++;
                        h_split();
#if 0
                        if (x == 0 && y == 0)
                                for (i = 0; i < MAXVAL; i++) {
                                        printf("%4d: %8.2f %s\n", i, h0[i],
                                               graf(log(1 + h0[i]), 0, 10,
                                                    60));
                                }
                        MSG("%3d,%3d: %4.0f,%4.0f,%4.0f,%4.2f\n", x, y,
                            H_dark, H_thr, H_light,
                            (H_thr - H_dark) / (H_light - H_dark));
#endif
                        Icon[x + y * Iw] = l = H_light;;
                        t = H_thr;
                        if (t < l)
                                sn++, sl += l, st += t, stt += t * t, sll += l * l, stl += t * l;       /* linear regression */
                }
        }
...
void avoid_links(float C) {
        ...
        *q;                     /* for output */
        ...
        for (j = 0; j < YRES; j++) {
                ...
                memcpy(r2, r3, XRES + 2);
                if (j + 1 < YRES)
                        MC(r3, pixmap + (j + 1) * XRES);
                q = pixmap + j * XRES;
                memset(q, 0, XRES);
                for (i = 0, p1 = r1 + 1, p2 = r2 + 1, p3 = r3 + 1;
                     i < XRES; ++i, ++p1, ++p2, ++p3, ++q) {
                        v = *p2;
                        t = 0;
                        if (v <= thresh_val) {
...
void hqbin() {
        ...
        *q1, *q2;               /* for output on 2 rows */
        ...
        for (j = 0; j < YRES; j++) {
                ...
                q2 = q1 + bpl;
                memset(q1, 0, 2 * bpl);
                for (i = 0, m = 128, p1 = r1 + 1, p2 = r2 + 1, p3 = r3 + 1;
                     i < XRES; ++i, ++p1, ++p2, ++p3) {
