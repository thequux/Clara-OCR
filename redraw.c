...
void draw_button(int i, int j, int w, int h, int p, char *l) {
        ...
        if (l != NULL) {
                ...
                if ((xti.nchars = strlen(xti.chars)) > 0)
                        XDrawText(xd, xw, xgc,
                                  i + (w - DFW * xti.nchars) / 2,
                                  j + (h - DFH) / 2 +
                                  (dfont->max_bounds).ascent, &xti, 1);
        }
}
...
void draw_bb(int x, int y, char *BL[], int b0) {
        ...
        for (i = j = 0; i < BUTT_PER_COL; ++i) {
                ...
                if ((redraw_button < 0) || (redraw_button == i + b0)) {
                        XSetForeground(xd, xgc, gray.pixel);
                        draw_button(x, y + i * (BH + VS), BW, BH, (!f) &&
                                    (button[i + b0] != 0), NULL);
                        XSetForeground(xd, xgc, black.pixel);
                        XDrawText(xd, xw, xgc,
                                  x + (BW - DFW * xti.nchars) / 2,
                                  y + i * (BH + VS) + (BH - DFH) / 2 +
                                  (dfont->max_bounds).ascent, &xti, 1);
                }
        }
        ...
        if (redraw_button < 0) {
                ...
                draw_frame(x - 1, y - 1, BW + 2,
                           BUTT_PER_COL * (BH + VS) - VS + 2, 1);
        }
}
...
void draw_tab(int x, int y, int t) {
        ...
        XSetForeground(xd, xgc, gray.pixel);
        XFillRectangle(xd, xw, xgc, x, y, TW,
                       (tab != t) ? TH - 2 : TH + 1);
        XSetForeground(xd, xgc, black.pixel);
        XDrawText(xd, xw, xgc, x + (TW - 4 - DFW * xti.nchars) / 2,
                  y + DFH, &xti, 1);

        /* corners */
        XSetForeground(xd, xgc, darkgray.pixel);
        for (k = 0; k < 5; ++k) {
                XDrawPoint(xd, xw, xgc, x + arc[2 * k],
                           y + arc[2 * k + 1]);
                XDrawPoint(xd, xw, xgc, x + TW - 1 - arc[2 * k],
                           y + arc[2 * k + 1]);
        }
        ...
        if (tab == t) {
                ...
                XSetForeground(xd, xgc, white.pixel);
                for (k = 0; k < 7; ++k)
                        XDrawPoint(xd, xw, xgc, x + arc2[2 * k],
                                   y + arc2[2 * k + 1]);
                XSetLineAttributes(xd, xgc, 2, LineSolid, CapNotLast,
                                   JoinRound);
                ...
                XDrawLine(xd, xw, xgc, x + TW, PT + TH, PM + PW - 1,
                          PT + TH);

                /* shadowed portion of the frame */
                XSetForeground(xd, xgc, vdgray.pixel);
                for (k = 0; k < 7; ++k)
                        XDrawPoint(xd, xw, xgc, x + TW - 1 - arc2[2 * k],
                                   y + arc2[2 * k + 1]);
                XDrawLine(xd, xw, xgc, x + TW - 1, y + 4, x + TW - 1,
                          y + TH + 1);
                XSetLineAttributes(xd, xgc, 1, LineSolid, CapNotLast,
                                   JoinRound);
        }
}
#endif
...
void draw_pm(unsigned char *r, int RCX, int RCY, int x, int y) {
        ...
        if ((disp_opt == 0) || (pm_t == 1)) {
                ...
                if (pm_t == 8) {
                        ...
                        for (np = 0, i = 0, u = X0; (u < RCX) && (i < DW);
                             ++i, u += rf) {
                                for (j = 0, v = Y0; (v < RCY) && (j < DH);
                                     ++j, v += rf)
                                        if (r[u + v * RCX] < T) {
        ...
        /* optimized per-depth code */
                ...
                if ((ocring) &&
                    (onlystep == OCR_OTHER) && (ocr_other == 3)) {
                ...
                if ((DW != cw) || (DH != ch)) {
                        ...
                        image =
                            XCreateImage(xd, tv, d, ZPixmap, 0, NULL, DW,
                                         DH, pad, bpl);
                }
...
void draw_stepper(int x, int y, int o, int RW) {
        ...
        if (o == 9) {
                ...
                /*
                   ...
                   ***
                 */
                XDrawLine(xd, xw, xgc, x + RW - 2, y - m, x + RW - 2,
                          y + m);
                XDrawLine(xd, xw, xgc, x + RW - 3, y - m + 1, x + RW - 3,
                          y + m - 1);
        }
        ...
        /* draw stepper pointing to top (12 o'clock), see comments above */
                ...
                XDrawLine(xd, xw, xgc, x, y, x + m, y + RW - 2);
                XDrawLine(xd, xw, xgc, x, y + 1, x + m - 1, y + RW - 2);
                XDrawLine(xd, xw, xgc, x - m, y + RW - 2, x + m,
                          y + RW - 2);
                XDrawLine(xd, xw, xgc, x - m + 1, y + RW - 3, x + m - 1,
                          y + RW - 3);
        }
...
void draw_sbars(void) {
         ...
         */
        XSetForeground(xd, xgc, darkgray.pixel);
        XFillRectangle(xd, xw, xgc, DM - 1, DT + DH + 9, a - DM + 1,
                       RW + 2);
        XFillRectangle(xd, xw, xgc, a + b, DT + DH + 9,
                       DM - 1 + DW + 2 - a - b, RW + 2);
         ...
         */
        XSetForeground(xd, xgc, darkgray.pixel);
        XFillRectangle(xd, xw, xgc, DM + DW + 9, DT - 1, RW + 2,
                       a - DT + 1);
        XFillRectangle(xd, xw, xgc, DM + DW + 9, a + b, RW + 2,
                       DT - 1 + DH + 2 - a - b);
...
void draw_mbar(void) {
        ...
        for (i = n = 0, p = 1; i <= TOP_CM; ++i) {
                if (CM[i].a == 1) {
                        ...
                        if (xti.nchars > 0)
                                XDrawText(xd, xw, xgc, p * DFW, DFH + 2,
                                          &xti, 1);
                        CM[i].p = p;
...
void draw_menu(void) {
        ...
        if (CY0_orig < 0) {
                ...
                else if ((cmenu->a == 2) && (0 <= curr_hti) &&
                         (curr_hti <= topge)) {
                        CX0 =
                            DM + ge[curr_hti].l - X0 + (ge[curr_hti].r -
                                                        ge[curr_hti].l -
                                                        CW) / 2;
        ...
        for (i = 0; i < cmenu->n; ++i) {
                ...
                if ((cmenu->t)[i] & CM_NG) {
                        XSetLineAttributes(xd, xgc, 1, LineSolid, CapButt,
                                           JoinRound);
                        XSetForeground(xd, xgc, darkgray.pixel);
                        XDrawLine(xd, xw, xgc, CX0, y + 1, CX0 + CW,
                                  y + 1);
                        XSetForeground(xd, xgc, black.pixel);
                }
        }
...
void draw_cfont_at(int i, int j, int d) {
        ...
        if (p == WHITE) {
                XSetForeground(xd, xgc, white.pixel);
                XFillRectangle(xd, xw, xgc, DM + i * GS + GW,
                               DT + j * GS + GW, GS - GW, GS - GW);
        }
        ...
        if (((cfont[i + j * HR]) & D_MASK) == D_MASK) {
                ...
                XDrawLine(xd, xw, xgc, DM + i * GS + 1, DT + j * GS + 1,
                          DM + (i + 1) * GS, DT + (j + 1) * GS);
                XDrawLine(xd, xw, xgc, DM + (i + 1) * GS, DT + j * GS,
                          DM + i * GS + 1, DT + (j + 1) * GS - 1);
        }
}
...
void draw_cfont(char *cfont) {
        ...
        if (redraw_grid != 0) {
                ...
                /* grid vertical lines */
                for (i = 0; i <= HR; ++i)
                        XDrawLine(xd, xw, xgc, DM + i * GS, DT,
                                  DM + i * GS, DT + DH);

                /* grid horizontal lines */
                for (j = 0; j <= VR; ++j)
                        XDrawLine(xd, xw, xgc, DM, DT + j * GS, DM + DW,
                                  DT + j * GS);
                redraw_grid = 0;
        }
        ...
        for (i = 0; i < HR; ++i) {
                for (j = 0; j < VR; ++j) {
                        if (((p = cfont[i + j * HR]) & D_MASK) == D_MASK) {
                                ...
                                XDrawLine(xd, xw, xgc, DM + i * GS + 1,
                                          DT + j * GS + 1,
                                          DM + (i + 1) * GS,
                                          DT + (j + 1) * GS);
                                XDrawLine(xd, xw, xgc, DM + (i + 1) * GS,
                                          DT + j * GS, DM + i * GS + 1,
                                          DT + (j + 1) * GS - 1);
                        }
                }
...
*/
void draw_rb(unsigned char *bm, int bpl, int rh, int x, int y, int w,
             int h, int dx0, int dy0, int rf) {
        int a, k, i, j, np, c;
        unsigned char *p, m;
        XPoint xp[100];
...
void draw_closure(cldesc *b, int a) {
        ...
        if (a <= 0) {
                ...
                if (a == 0)
                        ...
                        XSetForeground(xd, xgc, darkgray.pixel);
                draw_rb(b->bm, bpl, 0, x - b->l, y - b->t, w, h, dx0, dy0,
                        RF);
        }
}
...
void draw_symbol(sdesc *s, int a) {
        ...
        if (*cm_d_bb != ' ') {
                ...
                if ((a == -3) || (a == -4)) {
                        XSetForeground(xd, xgc, white.pixel);
                        XFillRectangle(xd, xw, xgc, l, t, r - l + 1,
                                       b - t + 1);
                }
                if (a != -4) {
                        XSetForeground(xd, xgc, black.pixel);
...
void draw_inp(edesc *c) {
        ...
        if ((xti.nchars = strlen(d)) > 0)
                XDrawText(xd, xw, xgc, x, y + (dfont->max_bounds).ascent,
                          &xti, 1);
        ...
        /* oh! ugly! */
        if (use_xb) {
                XCopyArea(xd, xw, XW, xgc, x - 1, y - 1, dx + 2, dy + 2,
                          x - 1, y - 1);
        }
}
...
void draw_ge(edesc *t) {
        ...
        if (t->type == GE_TEXT) {
                ...
                if (text_switch) {
                        ...
                        if ((t->a) && ULINK) {
                                XSetLineAttributes(xd, xgc, 1, LineSolid,
        ...
        if (t->type == GE_IMG) {
                ...
                if (strcmp(t->txt, "internal") == 0)
                        draw_bm(rw, RWX, RWY, DM + t->l - X0 + 2,
                                DT + t->t - Y0 + 2);
                ...
                /* draw pattern */
                        ...
                        if ((0 <= i) && (i <= topp)) {
                                ...
                                XFillRectangle(xd, xw, xgc,
                                               ...
                                               FS / plist_rf);
                                XSetForeground(xd, xgc, black.pixel);
                                draw_rb(pattern[i].b, FS / 8, 0, 0, 0,
                                        FS / plist_rf - 1,
                                        FS / plist_rf - 1,
                                        DM + t->l - X0 + 2,
                                        DT + t->t - Y0 + 2, plist_rf);
                        }
                }
                ...
                /* draw aligned pattern */
                        ...
                        if ((0 <= i) && (i <= topp)) {
                                ...
                                XFillRectangle(xd, xw, xgc,
                                               ...
                                               FS / plist_rf);
                                XSetForeground(xd, xgc, black.pixel);
                                draw_rb(pattern[i].b, FS / 8, 0, 0, 0, w,
                                        FS / plist_rf - 1,
                                        DM + t->l - X0 + 2,
                                        DT + t->t - Y0 + 2, plist_rf);
                        }
                ...
                /* draw word */
                        ...
                        XSetForeground(xd, xgc, white.pixel);
                        XFillRectangle(xd, xw, xgc, DM + t->l - X0 + 2,
                                       DT + t->t - Y0 + 2,
                                       (W->r - W->l + 1) / plist_rf,
                                       (W->b - W->t + 1) / plist_rf);
        ...
        /* draw rectangle background */
                ...
                if (t->bg >= 0) {
                        ...
                        w = t->r - t->l;
                        h = t->b - t->t;
                        XFillRectangle(xd, xw, xgc, DM + t->l - X0,
                                       DT + t->t - Y0, w, h);
                }
        }
...
void hl_gc(edesc *c) {
        ...
        if ((c->type == GE_TEXT) &&
            ((c->a == HTA_HREF) || (c->a == HTA_HREF_C))) {
                ...
                clip_dw(1);
                XSetForeground(xd, xgc, white.pixel);
                XFillRectangle(xd, xw, xgc, DM + l - X0, DT + t - Y0,
                               r - l, b - t);
                ...
                if (use_xb) {
                        XCopyArea(xd, xw, XW, xgc, DM + l - X0,
                                  DT + t - Y0, r - l, b - t, DM + l - X0,
                                  DT + t - Y0);
                }
        }
        ...
        /* text input field */
                ...
                draw_inp(c);
                show_hint(0,
                          "Fill in and (optionally) press ENTER to consist");
        }
...
void draw_dw(int bg, int inp) {
        ...
        if (HTML) {
                ...
                if (bg) {
                        XSetForeground(xd, xgc, gray.pixel);
                        XFillRectangle(xd, xw, xgc, DM - 1, DT - 1, DW + 2,
                                       DH + 2);
                }
        ...
        /* draw the scanned page */
                ...
                /*
                   ...
                   if requested.
                 */
                else if ((sliding == 0) && (*cm_d_closures != ' ')) {
                        int i;
                 ...
                 */
                        ...
                        if ((0 <= curr_mc) && (curr_mc <= tops)) {
                                ...
                                for (lm = curr_mc; (a = lsymb(lm, 1)) >= 0;
                                     lm = a);

                                /* search rightmost symbol */
                                for (rm = curr_mc; (a = rsymb(rm, 1)) >= 0;
                                     rm = a);
...
void img_size(char *img, int *w, int *h) {
        if (strcmp(img, "internal") == 0) {
                *w = (RWX + 4 + 1) / plist_rf;
                *h = (RWY + 4 + 1) / plist_rf;
        } else if ((strncmp(img, "pattern/", 8) == 0) ||
                   (strncmp(img, "symbol/", 7) == 0)) {
                *w = FS / plist_rf;
                *h = FS / plist_rf;
...
void add_text_welcome(char *s, int y) {
        ...
        if (((DT - PT - TH) > 2 * DFH) && (DW > t * DFW)) {
                XDrawText(xd, xw, xgc, DM + (DW - t * DFW) / 2, y, &xti,
                          1);
        }
}
...
void draw_welcome(void) {
        ...
        add_text_welcome("(  C  L  A  R  A     O  C  R  )",
                         DT + DH + 10 + DFH + VS);
        snprintf(v, 19, "version %s", version);
        v[19] = 0;
        add_text_welcome(v, PT + PH - 6 * (DFH + VS));
        add_text_welcome
            ("(C) 1999-2002 Ricardo Ueda Karpischek and others",
             PT + PH - 5 * (DFH + VS));
        add_text_welcome("Clara OCR comes with ABSOLUTELY NO WARRANTY",
                         PT + PH - 4 * (DFH + VS));
        add_text_welcome("type L to see the conditions",
                         PT + PH - 3 * (DFH + VS));
...
void draw_map(void) {
        ...
        if (*cm_v_map != ' ') {
                XSetForeground(xd, xgc, darkgray.pixel);
                XFillRectangle(xd, xw, xgc, PM - 52, PT - 1, 42,
                               (max_sz * 15 + max_sz - 1) + 4);
        }
        ...
        if ((*cm_v_map != ' ') && (alpha != NULL)) {
                ...
                /* frames */
                if (alpha != NULL)
                        draw_dframe(PM - 50, PT + 1,
                                    alpha_cols * 12 + alpha_cols - 1,
                                    alpha_sz * 15 + alpha_sz - 1);
        }
}
...
void redraw_junction(int j, int oj, int tdw, int bdw) {
        ...
        /* repaint background avoiding to overwrite the components */
        {
                ...
                XFillRectangle(xd, xw, xgc, PM + 4, dw[bdw].dt - 10 + 2,
                               PW - 8, 6);
                ...
                if (tdw == PAGE) {
                        ...
                        if (j > oj) {
                                ...
                                XFillRectangle(xd, xw, xgc, PM + 4, b - 3,
                                               l, h);

                                /* right clearance */
                                XFillRectangle(xd, xw, xgc,
                                               dw[PAGE].dm + dw[PAGE].dw,
                                               b - 3, l, h);
                        }
                ...
                if (sb > 0) {
                        ...
                        XFillRectangle(xd, xw, xgc, PM + 4,
                                       dw[bdw].dt - 10 + 2, PW - 8, 6);

                        /* background between the top window and the scrollbar */
                        XFillRectangle(xd, xw, xgc, PM + 4,
                                       dw[bdw].dt - 10 - sb + 2, PW - 8,
                                       6);

                        /* background on the corner of the two scrollbars */
                        XFillRectangle(xd, xw, xgc,
                                       PM + PW - 10 - RW - 10 + 2,
                                       dw[bdw].dt - sb - 2, 8 + RW + 4,
                                       RW + 4);
                }
        }
...
void draw_zone(int *limits) {
         ...
         */
                ...
                for (j = 0; j < 4; ++j) {
                        ...
                        if (f) {
                                XDrawLine(xd, xw, xgc, DM + (fx - X0) / RF,
                                          DT + (fy - Y0) / RF,
                                          DM + (lx - X0) / RF,
                                          DT + (ly - Y0) / RF);
                        }
...
void redraw(void) {
        ...
        if ((redraw_tab) ||
            ...
            (redraw_zone) ||
            (redraw_stline) ||
            (redraw_pbar) || (redraw_flea) || (redraw_menu)) {
        ...
        if (redraw_bg) {
                ...
                /* a bad method.. */
                        ...
                        XFillRectangle(xd, xw, xgc, 0, a, 10 + BW + 10,
                                       WH - a);
                        a = MH + 10 + PH;
                        XFillRectangle(xd, xw, xgc, 0, a, WW, WH - a);
                        XFillRectangle(xd, xw, xgc, WW - 10, MH, 10,
                                       WH - MH);
                }
                ...
                XSetLineAttributes(xd, xgc, 2, LineSolid, CapNotLast,
                                   JoinRound);
                XSetForeground(xd, xgc, vdgray.pixel);
                XDrawLine(xd, xw, xgc, PM, PT + PH - 1, PM + PW - 1,
                          PT + PH - 1);
                XDrawLine(xd, xw, xgc, PM + PW - 1, PT + TH, PM + PW - 1,
                          PT + PH - 1);
                XSetForeground(xd, xgc, white.pixel);
                ...
                if (CDW == PAGE_FATBITS) {
                        ...
                        if (fun_code != 3) {
                                if ((0 <= last_fp) && (last_fp <= topfp)) {
                                        draw_flea(fp[2 * last_fp],
                                                  fp[2 * last_fp + 1], 0);
                                        last_fp = -1;
                                }
                                if ((0 <= curr_fp) && (curr_fp <= topfp))
                                        draw_flea(fp[2 * curr_fp],
                                                  fp[2 * curr_fp + 1], 0);
                        }
        ...
        if (redraw_j2) {
                redraw_junction(page_j2, opage_j2, PAGE_OUTPUT,
                                PAGE_SYMBOL);
        }
        ...
        if (redraw_stline) {
                ...
                /* clear message area */
                XSetForeground(xd, xgc, darkgray.pixel);
                XFillRectangle(xd, xw, xgc, 10, WH - 10 - DFH - 2, WW - 20,
                               DFH + 4);
                ...
                xti.chars = mb;
                if ((xti.nchars = strlen(xti.chars)) > 0) {
                        XDrawText(xd, xw, xgc, 10, WH - 10 - DFH + DFA,
                                  &xti, 1);
                }
        }
        ...
        if (redraw_pbar) {
                ...
                XSetLineAttributes(xd, xgc, 2, LineSolid, CapNotLast,
                                   JoinRound);
                XSetForeground(xd, xgc, black.pixel);
                XDrawLine(xd, xw, xgc, 10, WH - 10, 10 + redraw_pbar,
                          WH - 10);
                XSetLineAttributes(xd, xgc, 1, LineSolid, CapNotLast,
                                   JoinRound);
        }
        ...
        if (use_xb) {
                ...
                /* copy the menu */
                        ...
                        if (cmenu != NULL) {
                                XCopyArea(xd, xw, XW, xgc, CX0 - 1,
                                          CY0 - 1, CW + 2, CH + 2, CX0 - 1,
                                          CY0 - 1);
                        }
                }
                        ...
                        if (redraw_menu) {
                                XCopyArea(xd, xw, XW, xgc, 0, 0, WW, MH, 0,
                                          0);
                        ...
                        /* copy the buttonbar */
                        if (redraw_button) {
                                XCopyArea(xd, xw, XW, xgc, 9, PT - 1,
                                          BW + 2,
                                          BUTT_PER_COL * (BH + VS) - VS +
                                          2, 9, PT - 1);
                        }

                        /* copy the tabs */
                        if (redraw_tab) {
                                XCopyArea(xd, xw, XW, xgc, PM - 2, PT,
                                          PW + 4, TH, PM - 2, PT);
                        }
                         ...
                         */
                        if ((redraw_dw) ||
                            (redraw_j1) || (redraw_j2) || (redraw_inp)) {

                                XCopyArea(xd, xw, XW, xgc, PM - 2, PT + TH,
                                          PW + 4, PH - TH, PM - 2,
                                          PT + TH);
                        }
                        ...
                        if (redraw_stline) {
                                XCopyArea(xd, xw, XW, xgc, 10,
                                          WH - 10 - DFH - 2, WW - 20,
                                          DFH + 4, 10, WH - 10 - DFH - 2);
                        }
        ...
        /* resizing PAGE/PAGE_OUTPUT junction */
                ...
                if ((y > PT + TH + 2 * RW + 20 + 10 + RW) &&
                    (y < page_j2 - 2 * RW - 20 - 10 - RW)) {
                        ...
                        cur_dw = CDW;
                        CDW = PAGE;
                        if ((DT == LAST_PAGE_DT) && (DM == LAST_PAGE_DM)) {
                                redraw_j1 = 1;
                                redraw_zone = 1;
                        } else
                                force_redraw();
...
void setview(int mode) {
        ...
        if (mode == WELCOME) {
                ...
                /* window margins */
                DM = PM + (PW - DW) / 2;
                DT = PT + TH + (PH - TH - DH - 8 * (DFH + VS)) / 2 + DFH +
                    VS;
         ...
         */
                 ...
                 (mode == PAGE_LIST) ||
                 (mode == PAGE_DOUBTS) ||
                 (mode == TUNE) || (mode == TUNE_ACTS)) {

                /* current window and tab */
                CDW = mode;
                if ((mode == PATTERN) ||
                    (mode == PATTERN_LIST) || (mode == PATTERN_TYPES)) {
                        ...
                        tab = PAGE;
                           ...
                           (mode == PATTERN_ACTION) || (mode == DEBUG)) {
        ...
        /* to PAGE or PAGE_OUTPUT or PAGE_SYMBOL */
                 ...
                 (mode == PAGE_OUTPUT) || (mode == PAGE_SYMBOL)) {
